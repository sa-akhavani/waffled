...
class InputTree:
    ...
    def tree_to_request(self, partial=False):
        ...
        return self.request.replace(b'_LENGTH_', str(len(self.request) - len(self.request.split(b'\r\n\r\n')[0]) - 4).encode('utf-8') ).replace(b'_URI_', self.uri.encode('utf-8')).replace(b'_HOST_', self.host_header.encode('utf-8')).replace(b'_REQUEST_ID_', str(self.seed).encode('utf-8'))
